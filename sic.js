...
//http://www.terminally-incoherent.com/stuff/projects/sic/sic.sql
// view source
// copy/paste here
const sic = `
...
const parsed = sic.split('\n').filter(r => !r.match(/^$/)).map(r => r.trim()).filter(r => r.match(/\d/)).map(r => {
	...
	const cleanWords = words.map(w => {
		...
		const word = [ 'and', 'of' ].includes(lower) ? lower : lower.substr(0, 1).toUpperCase() + lower.substr(1);
		return word;
	});
	return { sic, description: cleanWords.join(' ') };
});

console.log(`CREATE DATABASE IF NOT EXISTS ${database};`);
console.log(`DROP TABLE IF EXISTS ${database}.sic;`);
console.log(`CREATE TABLE ${database}.sic (
  code varchar(8) NOT NULL,
  description varchar(100) NOT NULL,
  PRIMARY KEY  (code)
) ENGINE=InnoDB;`);

parsed.forEach(({ sic, description }) => {
  if (sic) {
    description = description.replace(/"/g, '').trim();;
    console.log(`INSERT IGNORE INTO ${database}.sic (code, description) values ("${sic}", "${description}");`);
  }
});
